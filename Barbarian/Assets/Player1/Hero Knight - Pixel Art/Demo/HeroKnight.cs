...
public class HeroKnight : MonoBehaviour {
    ...
    void Update ()
    {
       ...
       // else if (Input.GetKeyDown("q") && !m_rolling)
        ...
        //Attack
        if(Input.GetKeyDown("f") && m_timeSinceAttack > 0.25f && !m_rolling)
        {
            m_currentAttack++;
            ...
            Collider2D[] hitenemy = Physics2D.OverlapCircleAll(AttackPointP1.position, attackRange, enemyLayer);
            foreach(Collider2D Player2Layer in hitenemy){
                Debug.Log("Hit " + Player2Layer.name);

                // Send the damage over to the player 
                Player2Layer.GetComponent<HeroKnight_2>().p2TakeDamage((double)20.0);
            }
        ...
        if (Input.GetKeyDown("c") && !m_rolling && canShootFireball){
        ...
        // Block
        else if (Input.GetKeyDown("g") && !m_rolling)
        {
     ...
     public void p1TakeDamage(double damage){
